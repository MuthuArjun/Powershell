$servers=@("isa1")
$file = "C:\users\jevenson\Desktop\Log_files.txt"

foreach ($server in $servers)
{

$local_disk = (GET-WMIOBJECT -query "SELECT * from win32_logicaldisk where DriveType = 3" -ComputerName $server).DeviceID
...
foreach ($disk in $local_disk)
    {
        ...
        $dir = gci \\$server\$ldisk`$\ | where {$_.Name -notlike "Windows" -and $_.PSIsContainer} | where {$_.Name -notlike "Program Files*"} | gci -Include *log* -Recurse -ErrorAction SilentlyContinue | % {$_.DirectoryName} | Get-Unique
